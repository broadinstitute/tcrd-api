...
object DataSurveyor {
  ...
  object FieldPoll {
    def couldBeNumber(string: String): Boolean = Try(string.toDouble).isSuccess
  ...
  case class DataPoll(fields: Map[String, FieldPoll]) {
  ...
  object DataPoll {
    val empty: DataPoll = DataPoll(Map.empty)
  ...
  def fromData(recordIter: Iterator[Either[String, Record]]): DataPoll = {
    var dataPoll: DataPoll = DataPoll.empty
    recordIter.foreach {
      case Left(message) => println(message)
      case Right(record) => dataPoll += DataPoll.fromRecord(record)
    }
    dataPoll
  }
